...
/**
* Class representing putein ends.
* Scores a number of acid strings like AcidMotif, and chooses the best.
* The acid strings are fetched from an alignment file.
*/
public class PuteinEndMotif extends OrdinaryMotif {

/**
* @return String identifying this class = "PUE".
*/
...
*/
	public static final int PUTEINENDLENGTH = 10;
...
*/
  public PuteinEndMotif(Database database, int motifID, char rvGenus, String alignmentName) throws RetroTectorException {
   	super(database, motifID, classType(), "D", String.valueOf(rvGenus), 5.5f, null, null, 100.0f, "", alignmentName, "");
		...
		MOTIFSTRANDS = new byte[ali.nrOfRows()][PUTEINENDLENGTH];
		ROWNAMES = new String[ali.nrOfRows()];
		String s;
   	for (int i=0; i<MOTIFSTRANDS.length; i++) {
			s = ali.getRow(i).getTrailingString(PUTEINENDLENGTH);
			if (s != null) {
...
*/
  public final Motif motifCopy() throws RetroTectorException {
		throw new RetroTectorException("PuteinEndMotif cannot be copied");
	} // end of motifCopy()
...
*/
	public final float scoreInAcidString(String targetString, int posInTarget) {
		while (posInTarget < 0) {
			posInTarget++;
			targetString = Compactor.UNKNOWNCHAR + targetString;
		}
...
// ambiguous acids = -1, ordinary acids 0-63, conserved acids = 64-127
